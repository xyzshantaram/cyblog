...
const init = async () => {
    const settings = await TauriStore.load('.settings.json');
    ...
    const { elt } = await Workspace.init({
        pat, useGithub, workspace, namespaced
    });

    cf.select({ s: '#root', single: true })?.append(elt as HTMLElement);
